...
def load_map_positions():
    if os.path.exists(MAP_FILE):
        try:
            with open(MAP_FILE) as f: return json.load(f)
        except: pass
    return {}
...
/* ── MAP TAB ── */
.map-container{display:flex;gap:0;height:calc(100vh - 160px);min-height:500px;border:1px solid var(--border);border-radius:10px;overflow:hidden;}
.map-viewport{flex:1;overflow:auto;background:#0a1218;position:relative;cursor:grab;}
.map-viewport:active{cursor:grabbing;}
.map-canvas{position:relative;display:inline-block;min-width:100%;min-height:100%;}
#map-bg{display:block;max-width:none;user-select:none;pointer-events:none;}
.map-sidebar{width:240px;background:var(--panel);border-left:1px solid var(--border);
...
.map-dev-item.dragging{opacity:.4;}
.map-dev-item.mapped{border-color:rgba(0,255,157,.3);background:rgba(0,255,157,.04);}
.map-dev-item.mapped .map-dev-name{color:var(--dim);}
.map-dev-dot{width:8px;height:8px;border-radius:50%;flex-shrink:0;}
.map-dev-dot.online{background:var(--accent2);box-shadow:0 0 5px var(--accent2);}
.map-dev-dot.offline{background:var(--danger);box-shadow:0 0 5px var(--danger);}
...
/* Map nodes */
.map-node{position:absolute;transform:translate(-50%,-50%);cursor:pointer;z-index:10;
  transition:transform .15s;}
.map-node:hover{transform:translate(-50%,-50%) scale(1.3);z-index:20;}
.map-node-dot{width:14px;height:14px;border-radius:50%;position:relative;}
.map-node.online .map-node-dot{background:var(--accent2);box-shadow:0 0 8px var(--accent2);
  animation:map-online-pulse 2.5s ease-in-out infinite;}
.map-node.offline .map-node-dot{background:var(--danger);box-shadow:0 0 8px var(--danger);
  animation:map-offline-pulse .8s ease-in-out infinite;}
.map-node.unknown .map-node-dot{background:var(--dim);}
@keyframes map-online-pulse{0%,100%{box-shadow:0 0 4px var(--accent2),0 0 0 0 rgba(0,255,157,0);}
  50%{box-shadow:0 0 8px var(--accent2),0 0 12px 4px rgba(0,255,157,.2);}}
@keyframes map-offline-pulse{0%,100%{box-shadow:0 0 4px var(--danger),0 0 0 0 rgba(255,45,85,0);opacity:1;}
  50%{box-shadow:0 0 12px var(--danger),0 0 20px 6px rgba(255,45,85,.3);opacity:.7;}}
.map-node-label{position:absolute;left:18px;top:50%;transform:translateY(-50%);
  font-size:.58rem;font-weight:600;white-space:nowrap;
  background:rgba(5,10,15,.85);padding:.1rem .35rem;border-radius:3px;
  pointer-events:none;border:1px solid rgba(255,255,255,.08);}
.map-node.online .map-node-label{color:var(--accent2);}
.map-node.offline .map-node-label{color:var(--danger);}
.map-node.unknown .map-node-label{color:var(--dim);}
/* Drag-over highlight */
.map-canvas.drag-over{outline:2px dashed var(--accent);outline-offset:-4px;}
/* Tooltip */
.map-tooltip{position:fixed;z-index:999;background:var(--panel);border:1px solid var(--border);
  border-radius:8px;padding:.7rem .9rem;min-width:180px;
...
.map-canvas.blank-bg{ background: #0a1218; }
.map-canvas.blank-bg::before{
...
#map-bg.hidden{ display:none !important; }
</style>
</head>
<body>
<header>
...
<div class="zoom-bar" id="zoom-bar-map" style="display:none;justify-content:space-between;">
  <div style="display:flex;align-items:center;gap:.6rem;">
    ...
    <input type="range" min="50" max="200" value="100" id="map-zoom-slider" oninput="setMapZoom(this.value)">
    <span id="map-zoom-lbl" style="font-family:'Share Tech Mono';font-size:.6rem;color:var(--accent);min-width:36px;">100%</span>
  </div>
  <div style="display:flex;gap:.5rem;align-items:center;">
    <button class="btn btn-blue" id="map-edit-btn" onclick="toggleMapEdit()">&#9998; Edit Mode</button>
    <button class="btn" id="map-bg-toggle" onclick="toggleMapBackground()" style="border:1px solid var(--border);">&#9974; Blank Canvas</button>
    <button class="btn btn-green" onclick="document.getElementById('map-upload-input').click()">&#8659; Upload Map Image</button>
    <input type="file" id="map-upload-input" accept="image/*" style="display:none" onchange="uploadMapImage(this)">
    <span style="font-family:'Share Tech Mono';font-size:.6rem;color:var(--dim);" id="map-edit-hint">Edit mode off — enable to place devices</span>
  </div>
</div>
<!-- Progress overlays -->
    ...
    <div class="map-container" id="map-container">
      <div class="map-viewport" id="map-viewport">
        <div class="map-canvas" id="map-canvas">
          <img id="map-bg" src="/map-image" alt="Site Map"
               onerror="this.style.display='none';document.getElementById('map-no-img').style.display='flex'">
          <div id="map-no-img" style="display:none;position:absolute;inset:0;align-items:center;justify-content:center;
               background:var(--panel2);border:2px dashed var(--border);border-radius:10px;flex-direction:column;gap:.8rem;">
            <div style="font-size:3rem;">&#128247;</div>
            <div style="font-size:.85rem;color:var(--dim);">No map image uploaded yet</div>
            <button class="btn btn-blue" onclick="document.getElementById('map-upload-input').click()">&#8659; Upload Floor Plan</button>
          </div>
      ...
      <div class="map-sidebar" id="map-sidebar">
        <div class="map-sidebar-hdr">
          <span style="font-size:.7rem;font-weight:700;letter-spacing:.1em;color:var(--accent);">DEVICES</span>
          <input id="map-dev-search" placeholder="Search..." oninput="renderMapSidebar()"
            ...
            border-radius:4px;color:var(--text);width:100%;margin-top:.4rem;">
        </div>
...
async function fetchMapPositions(){
  try{
    const r = await fetch('/map-positions');
    if(r.ok) mapPositions = await r.json();
    renderMapNodes();
  }catch{}
}
...
async function saveMapPositions(){
  try{
    await fetch('/map-positions',{method:'POST',
      headers:{'Content-Type':'application/json'},body:JSON.stringify(mapPositions)});
  }catch{}
}

let mapBgMode = 'image'; // 'image' | 'blank'
...
function setMapZoom(val){
  mapZoom = parseInt(val);
  document.getElementById('map-zoom-lbl').textContent = mapZoom+'%';
  const bg = document.getElementById('map-bg');
  if(bg){ bg.style.width=mapZoom+'%'; bg.style.maxWidth='none'; }
}
...
function toggleMapEdit(){
  ...
  if(mapEditMode){
    btn.style.cssText='border:1px solid var(--accent2)!important;color:var(--accent2);background:rgba(0,255,157,.07);';
    hint.textContent='Drag devices from list onto map to place them';
  } else {
    btn.style.cssText='';
    hint.textContent='Edit mode off — enable to place devices';
  }
  renderMapSidebar();
  renderMapNodes();
}
...
function getDeviceByKey(key){
  ...
  const section = parts[0], idx = parseInt(parts[1]);
  return (allDevices[section]||[])[idx]||null;
}
...
function renderMapSidebar(){
  ...
  for(const section of ['switches','aps']){
    (allDevices[section]||[]).forEach((d,i)=>{
      const key=`${section}:${i}`;
      const name=(d.name||'').toLowerCase(), ip=(d.ip||'').toLowerCase();
  ...
  list.innerHTML=items.map(({key,d,isMapped,status})=>`
    <div class="map-dev-item ${isMapped?'mapped':''}"
         draggable="${mapEditMode?'true':'false'}"
...
function renderMapNodes(){
  ...
  for(const [key,pos] of Object.entries(mapPositions)){
    ...
    node.innerHTML=`<div class="map-node-dot"></div><div class="map-node-label">${d.name||'?'}</div>`;
    node.addEventListener('mouseenter',e=>showMapTooltip(e,key,false));
    node.addEventListener('mouseleave',()=>{ if(activeTTNode!==key) hideMapTooltip(); });
    node.addEventListener('click',e=>{ e.stopPropagation(); showMapTooltip(e,key,true); });
    if(mapEditMode){
      node.draggable=true;
...
function removeMapNode(){
  ...
  const tt=document.getElementById('map-tooltip');
  tt.style.display='none'; tt.classList.remove('pinned');
  saveMapPositions(); renderMapNodes(); renderMapSidebar();
...
function initMapDrop(){
  ...
  canvas.addEventListener('drop',e=>{
    ...
    const bgRect=bg?bg.getBoundingClientRect():canvas.getBoundingClientRect();
    const x_pct=((e.clientX-bgRect.left)/bgRect.width*100).toFixed(2);
    const y_pct=((e.clientY-bgRect.top)/bgRect.height*100).toFixed(2);
    const [section,idx]=key.split(':');
    mapPositions[key]={x_pct:parseFloat(x_pct),y_pct:parseFloat(y_pct),section,idx:parseInt(idx)};
    saveMapPositions(); renderMapNodes(); renderMapSidebar();
...
async function uploadMapImage(input){
  ...
  reader.onload=async e=>{
    ...
    try{
      const r=await fetch('/map-image',{method:'POST',
        headers:{'Content-Type':'application/json'},
        body:JSON.stringify({image:b64,mime:file.type})});
      if(r.ok){
        document.getElementById('map-bg').src='/map-image?t='+Date.now();
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        if self.path=="/":
            ...
            self.send_json(data)
        elif self.path=="/map-positions":
            self.send_json(load_map_positions())
        elif self.path.startswith("/map-image"):
            # Serve the map image - check custom first, then default
            img_path = MAP_IMAGE if os.path.exists(MAP_IMAGE) else MAP_IMAGE+"_default"
...
if __name__=="__main__":
    ...
    print("="*55)
    # Copy bundled default map image if none uploaded yet
    import shutil, pathlib
    _script_dir = pathlib.Path(__file__).parent
    _default_map = _script_dir / "default_map.png"
    if _default_map.exists() and not os.path.exists(MAP_IMAGE):
        shutil.copy(str(_default_map), MAP_IMAGE)
        print("  Default map image loaded")
    load_warnings()
    run_all_tests()
