...
/* ── MAP TAB ── */
/* ── MAP TAB ── */
.map-container{display:flex;height:calc(100vh - 160px);min-height:500px;border:1px solid var(--border);border-radius:10px;overflow:hidden;}
...
.map-node{position:absolute;transform:translate(-50%,-50%);cursor:pointer;z-index:10;transition:transform .15s;}
.map-node:hover{transform:translate(-50%,-50%) scale(1.3);z-index:20;}
.map-node-dot{width:14px;height:14px;border-radius:50%;}
.map-node.online .map-node-dot{background:var(--accent2);box-shadow:0 0 8px var(--accent2);animation:map-on 2.5s ease-in-out infinite;}
.map-node.offline .map-node-dot{background:var(--danger);box-shadow:0 0 8px var(--danger);animation:map-off .8s ease-in-out infinite;}
.map-node.unknown .map-node-dot{background:var(--dim);}
@keyframes map-on{0%,100%{box-shadow:0 0 4px var(--accent2);}50%{box-shadow:0 0 14px 4px rgba(0,255,157,.25);}}
@keyframes map-off{0%,100%{box-shadow:0 0 4px var(--danger);opacity:1;}50%{box-shadow:0 0 16px 5px rgba(255,45,85,.35);opacity:.6;}}
.map-node-label{position:absolute;left:18px;top:50%;transform:translateY(-50%);font-size:.58rem;font-weight:600;
...
.map-node.connect-src .map-node-dot{box-shadow:0 0 0 3px var(--accent),0 0 12px var(--accent)!important;animation:none!important;}
...
<div class="zoom-bar" id="zoom-bar-map" style="display:none;justify-content:space-between;">
  ...
  <div style="display:flex;gap:.5rem;align-items:center;">
    ...
    <button class="btn" id="map-connect-btn" onclick="if(!mapEditMode){toast('Enable Edit Mode first','warn');}else{toggleConnectMode();}" style="border:1px solid var(--border);">&#9135; Connect</button>
...
function toggleMapEdit(){
  ...
  if(mapConnectMode && !mapEditMode) toggleConnectMode();
  ...
  if(mapEditMode){
    ...
    hint.textContent='Drag devices onto map — click nodes to connect';
  } else {
    btn.style.cssText='';
...
function toggleConnectMode(){
  ...
  if(mapConnectMode){
    btn.style.cssText='border:1px solid var(--warn)!important;color:var(--warn);background:rgba(255,204,0,.07);';
    document.getElementById('map-edit-hint').textContent='Connect mode: click first node, then second to draw a line';
  } else {
    btn.style.cssText='';
    document.getElementById('map-edit-hint').textContent='Drag devices onto map — click nodes to connect';
  }
  renderMapNodes();
}
...
function renderMapNodes(){
  ...
  for(const [key,pos] of Object.entries(mapPositions)){
    ...
    node.className=`map-node ${status}${isConnSrc?' connect-src':''}`;
