...
import subprocess, time, csv, json, os, threading, urllib.request, urllib.error, urllib.parse
...
API_TESTER_FILE = "api_tester.json"
WEB_PORT      = 8088
...
def add_warning(name, wtype, message, value=None):
    ...
    with _warn_lock:
        ...
        # Deduplicate: remove older entries of same name+type beyond 3
        same = [i for i,w in enumerate(_warnings) if w["name"]==name and w["type"]==wtype]
        for i in reversed(same[3:]):  # reverse to avoid index shifting
            del _warnings[i]
        del _warnings[50:]  # hard cap at 50 total
    _flush_warnings()
    return entry
...
def run_all_tests():
    ...
    if r["status"] == "ERROR":
        notify_bandwidth_fail(r.get("target",""), r.get("error","Unknown error"))
...
# Fail counters per device key — must hit threshold before alerting
_fail_counts   = {}   # key -> consecutive fail count
_online_states = {}   # key -> last known bool state (None = unknown)
_notif_times   = {}   # key -> last notification time (cooldown)
_notif_lock    = threading.Lock()

# Map alert queue — frontend polls this for banner notifications
import collections
_map_alerts = collections.deque(maxlen=20)  # last 20 alerts
_map_alerts_lock = threading.Lock()

def push_map_alert(kind, title, message):
    """Add an alert to the map banner queue."""
    with _map_alerts_lock:
        _map_alerts.append({
...
# Thresholds: switches=2 pings, everything else=3 pings
def _fail_threshold(section):
    return 2 if section == "switches" else 3

def _get_ha_notify_services():
    """Discover available notify services from HA, prefer mobile_app ones."""
    try:
        token = os.environ.get("SUPERVISOR_TOKEN","")
        if not token: return ["notify"]
        ...
        # Find all notify domain services
        notify_svcs = []
        for svc in services:
            if svc.get("domain") == "notify":
                for name in svc.get("services", {}).keys():
                    notify_svcs.append(name)
        # Prefer mobile_app services (HA companion app)
        mobile = [s for s in notify_svcs if s.startswith("mobile_app_")]
        if mobile: return mobile
        if notify_svcs: return notify_svcs
        return ["notify"]
    except Exception as e:
        print(f"  [NOTIFY] Could not discover services: {e}")
        return ["notify"]

_notify_services_cache = None
_notify_cache_time = 0

def send_ha_notification(title, message, tag=None):
    """Send push notification to all HA mobile app devices."""
    global _notify_services_cache, _notify_cache_time
    try:
        token = os.environ.get("SUPERVISOR_TOKEN","")
        if not token:
            print(f"  [NOTIFY] No SUPERVISOR_TOKEN — {title}: {message}")
            return
        # Refresh service list every 10 minutes
        now = time.time()
        if _notify_services_cache is None or (now - _notify_cache_time) > 600:
            _notify_services_cache = _get_ha_notify_services()
            _notify_cache_time = now
            print(f"  [NOTIFY] Services discovered: {_notify_services_cache}")
        payload = {"title": title, "message": message}
        if tag: payload["data"] = {"tag": tag, "push": {"sound": "default"}}
        sent = 0
        for svc in _notify_services_cache:
            try:
                req = urllib.request.Request(
                    f"http://supervisor/core/api/services/notify/{svc}",
                    data=json.dumps(payload).encode(),
                    headers={"Authorization": f"Bearer {token}", "Content-Type": "application/json"},
                    method="POST"
                )
                with urllib.request.urlopen(req, timeout=5):
                    print(f"  [NOTIFY] Sent via notify/{svc}: {title}")
                    sent += 1
            except Exception as e:
                print(f"  [NOTIFY] Failed via notify/{svc}: {e}")
        if not sent:
            print(f"  [NOTIFY] No services succeeded for: {title}")
    except Exception as e:
...
def send_sms(message):
    """Send SMS via Twilio."""
    cfg = load_config()
    if not cfg.get("sms_enabled"): return
    sid   = cfg.get("sms_account_sid","").strip()
    token = cfg.get("sms_auth_token","").strip()
    from_ = cfg.get("sms_from","").strip()
    to_   = cfg.get("sms_to","").strip()
    if not all([sid, token, from_, to_]):
        print("  [SMS] Credentials missing — skipping")
        return
    try:
        import base64
        url  = f"https://api.twilio.com/2010-04-01/Accounts/{sid}/Messages.json"
        data = urllib.parse.urlencode({"From": from_, "To": to_, "Body": message}).encode()
        ...
        with urllib.request.urlopen(req, timeout=10) as r:
            print(f"  [SMS] Sent to {to_}")
    except Exception as e:
        print(f"  [SMS] Failed: {e}")

def check_device_state_change(key, section, name, ip, is_online):
    """Track state changes and fire notifications when thresholds are met."""
    with _notif_lock:
        prev = _online_states.get(key, None)

        if not is_online:
            # Increment fail counter
            _fail_counts[key] = _fail_counts.get(key, 0) + 1
            count = _fail_counts[key]
            threshold = _fail_threshold(section)

            # Fire offline alert exactly when threshold is hit
            if count == threshold and prev is not False:
                _online_states[key] = False
                type_label = _get_type_label(name, section)
                push_map_alert("offline", f"⚠️ {type_label} Offline", f"{name} · {ip}")
                msg = f"⚠️ {type_label} Offline: {name} ({ip}) is not responding."
                threading.Thread(
                    target=send_ha_notification,
                    args=(f"⚠️ {type_label} Offline", f"{name} ({ip}) is not responding.", f"netmon_{key}"),
                    daemon=True
                ).start()
            ...
            _fail_counts[key] = 0
            if prev is False:
                # Was offline, now back — send recovery
                _online_states[key] = True
                type_label = _get_type_label(name, section)
                push_map_alert("online", f"✅ {type_label} Back Online", f"{name} · {ip}")
                msg = f"✅ {type_label} Back Online: {name} ({ip}) is responding again."
                threading.Thread(
                    target=send_ha_notification,
                    args=(f"✅ {type_label} Back Online", f"{name} ({ip}) is responding again.", f"netmon_{key}"),
                    daemon=True
                ).start()
                threading.Thread(target=send_sms, args=(msg,), daemon=True).start()
            elif prev is None:
...
def _get_type_label(name, section):
    n = name.upper()
    if "[TB]"     in n: return "Ticket Box"
    if "[KIOSK]"  in n: return "Kiosk"
    if "[OFFICE]" in n: return "Office"
    if "[SB]"     in n: return "Switch Box"
    if section == "aps": return "Access Point"
    return "Switch"

_last_outage_alert_time = 0  # cooldown tracker

def _check_mass_outage():
    """If 5+ devices are offline, send a network outage alert with 10min cooldown."""
    global _last_outage_alert_time
    recent_offline = sum(1 for k,v in _online_states.items() if v is False)
    if recent_offline >= 5:
        now = time.time()
        if now - _last_outage_alert_time > 600:  # 10 minute cooldown
            _last_outage_alert_time = now
            push_map_alert("outage", "🚨 Network Outage", f"{recent_offline} devices offline simultaneously")
            msg = f"🚨 Network Outage: {recent_offline} devices are currently offline."
            threading.Thread(
                target=send_ha_notification,
                args=(f"🚨 Network Outage Detected", f"{recent_offline} devices are currently offline. Possible network outage.", "netmon_outage"),
                daemon=True
            ).start()
            threading.Thread(target=send_sms, args=(msg,), daemon=True).start()

def notify_bandwidth_fail(url, error):
    """Log bandwidth failure — no push/SMS for bandwidth events."""
    push_map_alert("bw", "📉 Bandwidth Test Failed", error[:60])
    print(f"  [BW] Test failed: {error}")
...
def load_api_tester():
    if os.path.exists(API_TESTER_FILE):
        try:
            with open(API_TESTER_FILE) as f: return json.load(f)
        except: pass
    return {"saved": [], "history": []}

def save_api_tester(data):
    with open(API_TESTER_FILE,"w") as f:
        json.dump(data, f, indent=2)

def run_api_test(url, method="GET", headers_str="", body_str="", timeout=15):
    """Execute an API request and return full result dict."""
    import urllib.parse
    start = time.time()
    try:
        ...
        with urllib.request.urlopen(req, timeout=timeout) as r:
            ...
            except: body = raw.decode("latin-1","replace")
            # Try pretty JSON
            pretty = None
            if "json" in ct or (body.strip().startswith("{") or body.strip().startswith("[")):
                try: pretty = json.dumps(json.loads(body), indent=2)
                except: pass
            return {
                "ok": True,
                "status": r.status,
        ...
        elapsed = round((time.time()-start)*1000)
        try: body = e.read().decode("utf-8","replace")[:50000]
        except: body = ""
        return {
            "ok": False,
            "status": e.code,
    ...
    while True:
        try:
            ...
            for key, section, idx, d in targets:
                is_online         = ping_device(d["ip"])
                d["online"]       = is_online
                d["last_checked"] = datetime.now().isoformat()
                # Check for state change and fire HA notification if needed
                check_device_state_change(key, section, d.get("name","?"), d.get("ip",""), is_online)
...
/* ── API TESTER ── */
.at-rtab{font-family:'Exo 2';font-size:.7rem;font-weight:600;letter-spacing:.07em;
  padding:.45rem 1rem;cursor:pointer;color:var(--dim);border-bottom:2px solid transparent;
  text-transform:uppercase;transition:all .15s;}
.at-rtab:hover{color:var(--text);}
.at-rtab.active{color:var(--accent);border-bottom-color:var(--accent);}
.at-resp-panel{display:none;position:absolute;inset:0;overflow-y:auto;padding:.7rem;
  background:var(--panel2);border-radius:0 0 8px 8px;border:1px solid var(--border);border-top:none;}
.at-resp-panel.active{display:block;}
.at-hdr-row{display:grid;grid-template-columns:240px 1fr;gap:.3rem .8rem;padding:.2rem 0;
  border-bottom:1px solid rgba(255,255,255,.04);}
.at-hdr-key{color:var(--accent);font-size:.65rem;}
.at-hdr-val{color:var(--text);font-size:.65rem;word-break:break-all;}
.at-hist-item{padding:.5rem .7rem;border-radius:6px;border:1px solid var(--border);
  margin-bottom:.4rem;cursor:pointer;transition:border-color .15s;background:var(--panel);}
.at-hist-item:hover{border-color:var(--accent);}
.at-hist-status{font-family:'Share Tech Mono';font-size:.75rem;font-weight:700;}
.at-hist-meta{font-family:'Share Tech Mono';font-size:.58rem;color:var(--dim);margin-top:.15rem;}
.at-saved-item{padding:.4rem .6rem;border-radius:6px;border:1px solid var(--border);
  cursor:pointer;transition:border-color .15s;background:var(--panel);}
.at-saved-item:hover{border-color:var(--accent);}
.at-saved-name{font-size:.68rem;font-weight:600;color:var(--text);}
.at-saved-url{font-family:'Share Tech Mono';font-size:.56rem;color:var(--dim);
  overflow:hidden;text-overflow:ellipsis;white-space:nowrap;margin-top:.1rem;}
.at-method-badge{font-family:'Share Tech Mono';font-size:.58rem;font-weight:700;
  padding:.1rem .35rem;border-radius:3px;margin-right:.4rem;}
.at-method-GET{color:#00ff9d;background:rgba(0,255,157,.1);}
.at-method-POST{color:#ffd700;background:rgba(255,215,0,.1);}
.at-method-PUT{color:#fb923c;background:rgba(251,146,60,.1);}
.at-method-DELETE{color:#ff2d55;background:rgba(255,45,85,.1);}
.at-method-PATCH{color:#c084fc;background:rgba(192,132,252,.1);}
.at-method-HEAD{color:#00d4ff;background:rgba(0,212,255,.1);}
@keyframes spin{to{transform:rotate(360deg);}}
#map-alert-bar{position:absolute;top:.5rem;left:50%;transform:translateX(-50%);
  z-index:500;width:min(520px,calc(100% - 1rem));pointer-events:auto;display:none;}
#map-alert-summary{background:rgba(5,10,18,.96);border-radius:8px 8px 0 0;
  box-shadow:0 4px 24px rgba(0,0,0,.7);border:1px solid rgba(255,255,255,.08);
  cursor:pointer;user-select:none;display:flex;align-items:center;gap:.6rem;padding:.5rem .8rem;}
#map-alert-bar:not(.expanded) #map-alert-summary{border-radius:8px;}
#map-alert-summary:hover{border-color:var(--accent);}
#map-alert-expand{display:none;background:rgba(5,10,18,.97);border:1px solid rgba(255,255,255,.08);
  border-top:none;border-radius:0 0 8px 8px;max-height:320px;overflow-y:auto;
  box-shadow:0 8px 24px rgba(0,0,0,.7);}
#map-alert-bar.expanded #map-alert-expand{display:block;}
#map-alert-bar.expanded #map-alert-chevron{transform:rotate(180deg);}
.map-alert{padding:.45rem .8rem;display:flex;align-items:center;gap:.6rem;
  border-bottom:1px solid rgba(255,255,255,.04);}
.map-alert:last-child{border-bottom:none;}
.map-alert.ma-offline{border-left:3px solid var(--danger);}
.map-alert.ma-online {border-left:3px solid var(--accent2);}
.map-alert.ma-outage {border-left:3px solid #ff6b00;}
.map-alert.ma-bw     {border-left:3px solid var(--warn);}
.map-alert-icon{font-size:.95rem;flex-shrink:0;}
.map-alert-body{flex:1;min-width:0;}
.map-alert-title{font-family:'Exo 2';font-size:.68rem;font-weight:700;letter-spacing:.04em;}
.map-alert-msg{font-family:'Share Tech Mono';font-size:.57rem;color:var(--dim);
  white-space:nowrap;overflow:hidden;text-overflow:ellipsis;}
.map-alert-age{font-family:'Share Tech Mono';font-size:.54rem;color:var(--dim);flex-shrink:0;}
#map-alert-count{font-family:'Exo 2';font-size:.72rem;font-weight:800;}
#map-alert-chevron{font-size:.65rem;color:var(--dim);transition:transform .2s;margin-left:auto;flex-shrink:0;}
...
<header>
  <div class="logo">
    ...
    <div><h1>NETMONITOR</h1><small>AUTOMATED NETWORK DIAGNOSTICS v5.9</small></div>
  </div>
  <div class="hdr-r">
...
<div class="mob-nav" id="mob-nav">
  ...
  <div class="mob-nav-item" data-tab="apitester" onclick="mobNavGo('apitester')">
    <span class="mob-icon">&#128268;</span> API Tester
  </div>
...
<div class="tabs">
  ...
  <div class="tab"          data-tab="apitester" onclick="showTab('apitester',this)">&#128268; API Tester</div>
  ...
  <div style="display:flex;gap:.5rem;align-items:center;">
    ...
    <div style="position:relative;" id="map-edit-wrap">
      ...
      <div id="map-edit-menu" style="display:none;position:absolute;top:calc(100% + 6px);right:0;
        ...
        <div class="map-menu-item" onclick="clearMapLayout()" style="color:var(--danger);">&#128465; Clear All Node Positions</div>
    ...
    <button class="btn" onclick="testNotifications()"
      title="Test banners and push notifications"
      style="border:1px solid var(--dim);color:var(--dim);background:none;font-size:.68rem;">
      &#128276; Test
    </button>
...
<main>
  ...
  <div class="page" id="page-map">
    ...
    <!-- Map alert bar (collapsible) -->
    <div id="map-alert-bar" onclick="toggleMapAlertBar()">
      <div id="map-alert-summary">
        <span id="map-alert-icon">⚠️</span>
        <span id="map-alert-count" style="color:var(--danger);">0</span>
        <span id="map-alert-latest" style="font-family:'Share Tech Mono';font-size:.65rem;flex:1;
          white-space:nowrap;overflow:hidden;text-overflow:ellipsis;color:var(--text);"></span>
        <button onclick="clearMapAlerts(event)" title="Clear all"
          style="background:none;border:none;color:var(--dim);cursor:pointer;font-size:.7rem;
          padding:0 .3rem;flex-shrink:0;" >&#10005;</button>
        <span id="map-alert-chevron">&#9660;</span>
      </div>
      <div id="map-alert-expand"></div>
    </div>
  ...
  <!-- ── API TESTER PAGE ── -->
  <div class="page" id="page-apitester">
    <div style="display:flex;gap:1rem;height:calc(100vh - 200px);min-height:500px;">

      <!-- Left panel: request builder + saved -->
      <div style="width:320px;flex-shrink:0;display:flex;flex-direction:column;gap:.6rem;">

        <!-- Request builder -->
        <div class="mgr-card" style="flex-shrink:0;">
          <div style="display:flex;gap:.4rem;margin-bottom:.6rem;">
            <select id="at-method" style="font-family:'Share Tech Mono';font-size:.7rem;
            ...
            <input id="at-url" placeholder="https://api.example.com/endpoint"
              style="flex:1;font-size:.68rem;padding:.3rem .5rem;background:var(--panel2);
              border:1px solid var(--border);border-radius:4px;color:var(--text);">
          </div>
          <div style="margin-bottom:.4rem;">
            <label style="font-size:.6rem;color:var(--dim);letter-spacing:.08em;">HEADERS (one per line: Key: Value)</label>
            <textarea id="at-headers" rows="3" placeholder="Authorization: Bearer token&#10;Accept: application/json"
              style="width:100%;font-family:'Share Tech Mono';font-size:.62rem;padding:.3rem .5rem;
              background:var(--panel2);border:1px solid var(--border);border-radius:4px;
              color:var(--text);resize:vertical;box-sizing:border-box;margin-top:.2rem;"></textarea>
          </div>
          <div style="margin-bottom:.5rem;">
            <label style="font-size:.6rem;color:var(--dim);letter-spacing:.08em;">REQUEST BODY (POST/PUT)</label>
            <textarea id="at-body" rows="3" placeholder='{"key":"value"}'
              style="width:100%;font-family:'Share Tech Mono';font-size:.62rem;padding:.3rem .5rem;
              background:var(--panel2);border:1px solid var(--border);border-radius:4px;
              color:var(--text);resize:vertical;box-sizing:border-box;margin-top:.2rem;"></textarea>
          </div>
          <div style="display:flex;gap:.4rem;align-items:center;">
            <button class="btn btn-blue" onclick="runApiTest()" id="at-run-btn" style="flex:1;">
              &#9654; Send Request
            </button>
            <select id="at-timeout" title="Timeout"
              style="font-family:'Share Tech Mono';font-size:.65rem;background:var(--panel2);
              border:1px solid var(--border);color:var(--dim);border-radius:4px;padding:.3rem .4rem;">
              <option value="5">5s</option><option value="15" selected>15s</option>
              <option value="30">30s</option><option value="60">60s</option>
            </select>
            <button class="btn" onclick="saveApiRequest()" title="Save this request"
              style="border:1px solid var(--accent2);color:var(--accent2);font-size:.7rem;padding:.3rem .6rem;">
              &#128190;
            </button>
          </div>
        </div>
        ...
        <!-- Saved requests -->
        <div class="mgr-card" style="flex:1;display:flex;flex-direction:column;min-height:0;overflow:hidden;">
          <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:.5rem;">
            <span style="font-size:.65rem;font-weight:700;letter-spacing:.1em;color:var(--accent);">SAVED REQUESTS</span>
            <span style="font-size:.58rem;color:var(--dim);">click to load</span>
          </div>
          <!-- Pre-loaded RCS APIs -->
          <div id="at-saved-list" style="flex:1;overflow-y:auto;display:flex;flex-direction:column;gap:.3rem;min-height:0;"></div>
        </div>
      </div>

      <!-- Right panel: response -->
      <div style="flex:1;display:flex;flex-direction:column;min-width:0;gap:.6rem;">

        <!-- Status bar -->
        <div id="at-status-bar" style="display:none;" class="mgr-card">
          <div style="display:flex;gap:1.2rem;align-items:center;flex-wrap:wrap;">
            <span id="at-status-badge" style="font-family:'Share Tech Mono';font-size:.85rem;font-weight:700;"></span>
            <span id="at-status-text" style="font-size:.7rem;color:var(--dim);"></span>
            <span id="at-elapsed" style="font-family:'Share Tech Mono';font-size:.7rem;color:var(--accent);"></span>
            <span id="at-content-type" style="font-family:'Share Tech Mono';font-size:.62rem;color:var(--dim);"></span>
            <span id="at-ts" style="font-family:'Share Tech Mono';font-size:.58rem;color:var(--dim);margin-left:auto;"></span>
          </div>
        </div>

        <!-- Response tabs -->
        <div style="display:flex;gap:0;border-bottom:1px solid var(--border);">
          <div class="at-rtab active" id="at-rtab-pretty" onclick="switchRespTab('pretty')">Pretty</div>
          <div class="at-rtab" id="at-rtab-raw"    onclick="switchRespTab('raw')">Raw</div>
          <div class="at-rtab" id="at-rtab-headers" onclick="switchRespTab('headers')">Headers</div>
          <div class="at-rtab" id="at-rtab-history" onclick="switchRespTab('history')">History</div>
        </div>

        <!-- Response panels -->
        <div style="flex:1;min-height:0;position:relative;">
          <div id="at-resp-pretty" class="at-resp-panel active">
            <div id="at-pretty-content" style="font-family:'Share Tech Mono';font-size:.68rem;
              white-space:pre-wrap;word-break:break-all;line-height:1.6;color:var(--accent2);"></div>
          </div>
          <div id="at-resp-raw" class="at-resp-panel">
            <div id="at-raw-content" style="font-family:'Share Tech Mono';font-size:.65rem;
              white-space:pre-wrap;word-break:break-all;line-height:1.5;color:var(--text);"></div>
          </div>
          <div id="at-resp-headers" class="at-resp-panel">
            <div id="at-headers-content" style="font-family:'Share Tech Mono';font-size:.65rem;
              line-height:1.8;"></div>
          </div>
          <div id="at-resp-history" class="at-resp-panel">
            <div id="at-history-content"></div>
          </div>
          <!-- Loading spinner -->
          <div id="at-loading" style="display:none;position:absolute;inset:0;background:rgba(5,10,18,.85);
            align-items:center;justify-content:center;flex-direction:column;gap:.8rem;border-radius:8px;">
            <div style="width:36px;height:36px;border:3px solid var(--border);border-top-color:var(--accent);
              border-radius:50%;animation:spin 0.8s linear infinite;"></div>
            <span style="font-family:'Share Tech Mono';font-size:.7rem;color:var(--accent);">Sending request...</span>
          </div>
        </div>
      </div>
    </div>
  ...
  <div class="page" id="page-targets">
    ...
    <div class="mgr-grid">
      ...
      <div class="mgr-card">
        <h3 style="color:var(--accent2);">&#128241; Notifications</h3>
        <div class="add-form">
          <p style="font-size:.68rem;color:var(--dim);margin-bottom:.6rem;">HA push notifications are always on when the add-on has Supervisor access. SMS requires a Twilio account.</p>
          <div class="row">
            <div class="form-group">
              <label>SMS Enabled</label>
              <select id="sms-enabled"><option value="false">Disabled</option><option value="true">Enabled</option></select>
            </div>
            <div class="form-group">
              <label>Provider</label>
              <select id="sms-provider"><option value="twilio">Twilio</option></select>
            </div>
          </div>
          <div class="row">
            <div class="form-group"><label>Account SID</label><input id="sms-sid" placeholder="ACxxxxxxxxxxxxxxxx"></div>
            <div class="form-group"><label>Auth Token</label><input id="sms-token" type="password" placeholder="your_auth_token"></div>
          </div>
          <div class="row">
            <div class="form-group"><label>From Number</label><input id="sms-from" placeholder="+1xxxxxxxxxx"></div>
            <div class="form-group"><label>To Number</label><input id="sms-to" placeholder="+1xxxxxxxxxx"></div>
          </div>
          <div style="display:flex;gap:.5rem;">
            <button class="btn btn-green" onclick="saveSmsSettings()">Save SMS Settings</button>
          </div>
        </div>
        <div class="save-notice" id="notice-sms">&#10003; Saved</div>
        <!-- Test Panel -->
        <div style="margin-top:1rem;border-top:1px solid var(--border);padding-top:.8rem;">
          <div style="font-size:.65rem;font-weight:700;letter-spacing:.1em;color:var(--dim);text-transform:uppercase;margin-bottom:.6rem;">&#128270; Test Notifications</div>
          <div style="display:flex;gap:.5rem;flex-wrap:wrap;margin-bottom:.6rem;">
            <button class="btn btn-blue" onclick="testHaPush()" id="test-ha-btn">&#128241; Test HA Push</button>
            <button class="btn btn-blue" onclick="testSms()" id="test-sms-btn">&#128172; Test SMS</button>
            <button class="btn" onclick="testOfflineAlert()" style="border:1px solid var(--danger);color:var(--danger);background:rgba(255,45,85,.06);">&#128308; Simulate Device Offline</button>
            <button class="btn" onclick="testOnlineAlert()" style="border:1px solid var(--accent2);color:var(--accent2);background:rgba(0,255,157,.06);">&#128994; Simulate Device Online</button>
          </div>
          <div id="test-results" style="display:none;font-family:'Share Tech Mono';font-size:.65rem;border-radius:6px;padding:.6rem .8rem;background:var(--panel2);border:1px solid var(--border);">
            <div id="test-ha-result" style="margin-bottom:.25rem;"></div>
...
function renderTargetManager(cfg){
  ...
  const wsEl=document.getElementById('bw-warn-speed');if(wsEl)wsEl.value=cfg.warn_speed_mbps||1;
  const smsEn=document.getElementById('sms-enabled');if(smsEn)smsEn.value=cfg.sms_enabled?'true':'false';
  const smsSid=document.getElementById('sms-sid');if(smsSid)smsSid.value=cfg.sms_account_sid||'';
  const smsTok=document.getElementById('sms-token');if(smsTok)smsTok.value=cfg.sms_auth_token||'';
  const smsFrom=document.getElementById('sms-from');if(smsFrom)smsFrom.value=cfg.sms_from||'';
  const smsTo=document.getElementById('sms-to');if(smsTo)smsTo.value=cfg.sms_to||'';
}
...
function saveSmsSettings(){
  const cfg=JSON.parse(JSON.stringify(allConfig));
  cfg.sms_enabled  = document.getElementById('sms-enabled').value==='true';
  cfg.sms_provider = 'twilio';
  cfg.sms_account_sid = document.getElementById('sms-sid').value.trim();
  cfg.sms_auth_token  = document.getElementById('sms-token').value.trim();
  cfg.sms_from = document.getElementById('sms-from').value.trim();
  cfg.sms_to   = document.getElementById('sms-to').value.trim();
  saveConfigRemote(cfg,'sms');
}
async function testSms(){
  const btn = document.getElementById('test-sms-btn');
  const res = document.getElementById('test-sms-result');
  const panel = document.getElementById('test-results');
  btn.disabled=true; btn.textContent='Sending...';
  panel.style.display='block';
  res.innerHTML = '<span style="color:var(--dim);">&#9203; Sending test SMS...</span>';
  try{
    const r = await fetch(api('/sms-test'),{method:'POST',headers:{'Content-Type':'application/json'},body:'{}'});
    const d = await r.json();
    if(d.error){
      res.innerHTML = `<span style="color:var(--danger);">&#10007; SMS Failed: ${d.error}</span>`;
      toast('SMS test failed: '+d.error,'danger');
    } else {
      res.innerHTML = `<span style="color:var(--accent2);">&#10003; SMS sent to ${d.to||'your number'}</span>`;
      toast('&#128172; Test SMS sent!','ok');
    }
  }catch(e){
    res.innerHTML = `<span style="color:var(--danger);">&#10007; Error: ${e}</span>`;
    toast('SMS error: '+e,'danger');
  }
  btn.disabled=false; btn.textContent='&#128172; Test SMS';
}

async function testHaPush(){
  const btn = document.getElementById('test-ha-btn');
  const res = document.getElementById('test-ha-result');
  const panel = document.getElementById('test-results');
  btn.disabled=true; btn.textContent='Sending...';
  panel.style.display='block';
  res.innerHTML = '<span style="color:var(--dim);">&#9203; Sending HA push notification...</span>';
  try{
    const r = await fetch(api('/notify-test'),{method:'POST',headers:{'Content-Type':'application/json'},body:'{}'});
    const d = await r.json();
    if(!d.supervisor_token){
      res.innerHTML = '<span style="color:var(--warn);">&#9888; No SUPERVISOR_TOKEN — check add-on config has hassio_api: true</span>';
      toast('No Supervisor token','warn');
    } else {
      res.innerHTML = '<span style="color:var(--accent2);">&#10003; HA push sent — check your phone!</span>';
      toast('&#128241; HA push sent!','ok');
    }
  }catch(e){
    res.innerHTML = `<span style="color:var(--danger);">&#10007; Error: ${e}</span>`;
  }
  btn.disabled=false; btn.textContent='&#128241; Test HA Push';
}
...
async function testOfflineAlert(){
  ...
  panel.style.display='block';
  try{
    const r = await fetch(api('/notify-simulate'),{method:'POST',headers:{'Content-Type':'application/json'},
      body:JSON.stringify({type:'offline',name:'Test Device',ip:'10.0.0.1'})});
    const d = await r.json();
    toast('&#128308; Simulated offline alert fired','warn');
  }catch(e){ toast('Simulate failed: '+e,'danger'); }
}

async function testOnlineAlert(){
  const panel = document.getElementById('test-results');
  panel.style.display='block';
  try{
    const r = await fetch(api('/notify-simulate'),{method:'POST',headers:{'Content-Type':'application/json'},
      body:JSON.stringify({type:'online',name:'Test Device',ip:'10.0.0.1'})});
    const d = await r.json();
    toast('&#128994; Simulated online alert fired','ok');
  }catch(e){ toast('Simulate failed: '+e,'danger'); }
}
async function saveConfigRemote(cfg,noticeId){
  try{
    ...
    // Purge stale results then refresh dashboard so removed targets disappear immediately
    await fetch(api('/purge-results'),{method:'POST',headers:{'Content-Type':'application/json'},body:'{}'});
    try{renderDashboard(await(await fetch(api('/data'))).json());}catch{}
    toast('Saved','ok');
...
// ── API TESTER ──
let atData = {saved:[], history:[]};
let atCurrentResult = null;
let atRespTab = 'pretty';

// Pre-loaded RCS API endpoints
const RCS_PRESETS = [
  {name:'Ticket Box API',     method:'GET', url:'http://10.42.10.3/TBAPI/',            headers:''},
  {name:'Kiosk API',          method:'GET', url:'http://10.42.10.3/KIOSKAPI/',          headers:''},
  {name:'Attendant API',      method:'GET', url:'http://10.42.10.3/FunPassAttendant/ZpassAttendantWCF.svc', headers:''},
  {name:'Point of Sale API',  method:'GET', url:'https://rcs.zpassevents.com/ZpassPOS/FCPOSWCF.svc',       headers:''},
  {name:'Mobile API',         method:'GET', url:'https://web.rcsfunpass.com/FunPassCentralWCF.svc',        headers:''},
  {name:'HLSR Mobile API',    method:'GET', url:'https://rcs.zpassevents.com/HLSRMobile/HLSRCentralWCF.svc',headers:''},
  {name:'HLSR API',           method:'GET', url:'https://rcs.zpassevents.com/HLSRAPI/',                    headers:''},
];

async function initApiTester(){
  try{
    const r = await fetch(api('/api-tester'));
    if(r.ok) atData = await r.json();
  }catch{}
  renderAtSaved();
  renderAtHistory();
}
...
function switchRespTab(tab){
  atRespTab = tab;
  document.querySelectorAll('.at-rtab').forEach(t=>t.classList.remove('active'));
  document.querySelectorAll('.at-resp-panel').forEach(p=>p.classList.remove('active'));
  document.getElementById('at-rtab-'+tab).classList.add('active');
  document.getElementById('at-resp-'+tab).classList.add('active');
  if(tab==='history') renderAtHistory();
}

async function runApiTest(){
  const url    = document.getElementById('at-url').value.trim();
  const method = document.getElementById('at-method').value;
  const headers= document.getElementById('at-headers').value;
  const body   = document.getElementById('at-body').value;
  const timeout= document.getElementById('at-timeout').value;
  ...
  document.getElementById('at-status-bar').style.display='none';

  try{
    const r = await fetch(api('/api-tester/run'),{
      method:'POST',
      headers:{'Content-Type':'application/json'},
      body: JSON.stringify({url,method,headers,body,timeout:parseInt(timeout)})
    });
    const result = await r.json();
    atCurrentResult = result;
    atData.history = atData.history||[];
    atData.history.unshift(result);
    if(atData.history.length>100) atData.history.pop();
    renderAtResult(result);
    renderAtHistory();
  }catch(e){
...
function renderAtResult(r){
  // Status bar
  const bar = document.getElementById('at-status-bar');
  bar.style.display='block';
  const badge = document.getElementById('at-status-badge');
  badge.textContent = r.status || 'ERR';
  badge.style.color = r.ok ? 'var(--accent2)' : r.status>=400 ? 'var(--danger)' : 'var(--warn)';
  document.getElementById('at-status-text').textContent = r.status_text||'';
  document.getElementById('at-elapsed').textContent = r.elapsed_ms+'ms';
  document.getElementById('at-content-type').textContent = r.content_type||'';
  document.getElementById('at-ts').textContent = r.ts ? new Date(r.ts).toLocaleTimeString() : '';

  // Pretty
  const pretty = document.getElementById('at-pretty-content');
  if(r.pretty){
    pretty.textContent = r.pretty;
    pretty.style.color = 'var(--accent2)';
  } else if(r.error){
    pretty.textContent = '⚠️ '+r.error;
    pretty.style.color = 'var(--danger)';
  } else {
    pretty.textContent = r.body || '(empty response)';
    pretty.style.color = 'var(--text)';
  }

  // Raw
  document.getElementById('at-raw-content').textContent = r.body || r.error || '(empty)';

  // Headers
  const hc = document.getElementById('at-headers-content');
  hc.innerHTML = Object.entries(r.headers||{}).map(([k,v])=>
    `<div class="at-hdr-row"><span class="at-hdr-key">${k}</span><span class="at-hdr-val">${v}</span></div>`
  ).join('') || '<span style="color:var(--dim);font-size:.65rem;">No headers</span>';

  // Switch to pretty tab
  switchRespTab('pretty');
}

function renderAtHistory(){
  const el = document.getElementById('at-history-content');
  if(!el) return;
  const hist = atData.history||[];
  if(!hist.length){
    el.innerHTML='<div style="color:var(--dim);font-size:.65rem;padding:.5rem;">No history yet.</div>';
    return;
  }
  el.innerHTML = hist.slice(0,50).map((r,i)=>`
    <div class="at-hist-item" onclick="loadHistoryItem(${i})">
      <div style="display:flex;align-items:center;gap:.5rem;">
        <span class="at-method-badge at-method-${r.method||'GET'}">${r.method||'GET'}</span>
        <span class="at-hist-status" style="color:${r.ok?'var(--accent2)':r.status>=400?'var(--danger)':'var(--warn)'}">${r.status||'ERR'}</span>
        <span style="font-size:.65rem;color:var(--text);flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">${r.url||''}</span>
        <span style="font-family:'Share Tech Mono';font-size:.6rem;color:var(--dim);">${r.elapsed_ms}ms</span>
      </div>
      <div class="at-hist-meta">${r.ts ? new Date(r.ts).toLocaleString() : ''}</div>
    </div>`).join('');
}

function loadHistoryItem(i){
  const r = atData.history[i];
  if(!r) return;
  document.getElementById('at-url').value = r.url||'';
  document.getElementById('at-method').value = r.method||'GET';
  atCurrentResult = r;
  renderAtResult(r);
}

function renderAtSaved(){
  const el = document.getElementById('at-saved-list');
  if(!el) return;
  // Combine presets + user saved
  const userSaved = atData.saved||[];
  const allSaved = [
    ...RCS_PRESETS.map(s=>({...s, preset:true})),
    ...userSaved
  ];
  el.innerHTML = allSaved.map((s,i)=>`
    <div class="at-saved-item" onclick="loadSavedRequest(${i})">
      <div style="display:flex;align-items:center;justify-content:space-between;">
        <div>
          <span class="at-method-badge at-method-${s.method}">${s.method}</span>
          <span class="at-saved-name">${s.name}</span>
        </div>
        ${!s.preset?`<button onclick="deleteSavedRequest(event,${i-RCS_PRESETS.length})"
          style="background:none;border:none;color:var(--danger);cursor:pointer;font-size:.75rem;">&#10005;</button>`:''}
      </div>
      <div class="at-saved-url">${s.url}</div>
    </div>`).join('');
}

function loadSavedRequest(i){
  const allSaved = [...RCS_PRESETS, ...(atData.saved||[])];
  const s = allSaved[i];
  if(!s) return;
  document.getElementById('at-url').value    = s.url||'';
  document.getElementById('at-method').value = s.method||'GET';
  document.getElementById('at-headers').value= s.headers||'';
  document.getElementById('at-body').value   = s.body||'';
...
async function saveApiRequest(){
  ...
  if(!name) return;
  atData.saved = atData.saved||[];
  atData.saved.push({name, method, url,
    headers: document.getElementById('at-headers').value,
    body:    document.getElementById('at-body').value
  });
  await fetch(api('/api-tester/saved'),{method:'POST',
    headers:{'Content-Type':'application/json'},
...
async function deleteSavedRequest(e, i){
  e.stopPropagation();
  atData.saved.splice(i,1);
  await fetch(api('/api-tester/saved'),{method:'POST',
    headers:{'Content-Type':'application/json'},
    body:JSON.stringify(atData)});
  renderAtSaved();
...
// ── MAP ALERT BANNERS (collapsible bar) ──
let _seenAlertIds = new Set();
let _pendingMapAlerts = [];
let _mapAlertList = [];  // all current alerts

const ALERT_ICONS  = {offline:'⚠️', online:'✅', outage:'🚨', bw:'📉'};
const ALERT_CLASS  = {offline:'ma-offline', online:'ma-online', outage:'ma-outage', bw:'ma-bw'};
const ALERT_COLOR  = {offline:'var(--danger)', online:'var(--accent2)', outage:'#ff6b00', bw:'var(--warn)'};
const ALERT_SUMMARY_COLOR = {offline:'var(--danger)', online:'var(--accent2)', outage:'#ff6b00', bw:'var(--warn)'};

function showMapAlert(alert){
  _mapAlertList.unshift(alert);
  if(_mapAlertList.length > 30) _mapAlertList.pop();
  renderMapAlertBar();
}

function renderMapAlertBar(){
  const bar = document.getElementById('map-alert-bar');
  if(!bar) return;
  if(!_mapAlertList.length){ bar.style.display='none'; return; }
  bar.style.display='block';

  // Summary line — show most recent alert
  const top = _mapAlertList[0];
  document.getElementById('map-alert-icon').textContent = ALERT_ICONS[top.kind]||'ℹ️';
  const countEl = document.getElementById('map-alert-count');
  countEl.textContent = _mapAlertList.length > 1 ? `${_mapAlertList.length} alerts` : '';
  countEl.style.color = ALERT_SUMMARY_COLOR[top.kind]||'var(--danger)';
  document.getElementById('map-alert-latest').textContent = top.title + ' · ' + top.msg;
  document.getElementById('map-alert-summary').style.borderLeftColor = ALERT_SUMMARY_COLOR[top.kind]||'var(--danger)';
  document.getElementById('map-alert-summary').style.borderLeft = `3px solid ${ALERT_SUMMARY_COLOR[top.kind]||'var(--danger)'}`;

  // Expand list
  const expand = document.getElementById('map-alert-expand');
  expand.innerHTML = _mapAlertList.map((a,i)=>`
    <div class="map-alert ${ALERT_CLASS[a.kind]||''}">
      <span class="map-alert-icon">${ALERT_ICONS[a.kind]||'ℹ️'}</span>
      <div class="map-alert-body">
        <div class="map-alert-title" style="color:${ALERT_COLOR[a.kind]||'var(--accent)'}">${a.title}</div>
        <div class="map-alert-msg">${a.msg}</div>
      </div>
      <span class="map-alert-age">${new Date(a.ts).toLocaleTimeString([],{hour:'2-digit',minute:'2-digit',second:'2-digit'})}</span>
    </div>`).join('');
}

function toggleMapAlertBar(){
  document.getElementById('map-alert-bar').classList.toggle('expanded');
}

function clearMapAlerts(e){
  e.stopPropagation();
  _mapAlertList = [];
  const bar = document.getElementById('map-alert-bar');
...
async function pollMapAlerts(){
  try{
    const r = await fetch(api('/map-alerts'));
    if(!r.ok) return;
    ...
    for(const a of alerts){
      if(!_seenAlertIds.has(a.id)){
        ...
        if(mapActive){ showMapAlert(a); hasNew = true; }
        else _pendingMapAlerts.push(a);
      }
    }
  }catch(e){ console.warn('pollMapAlerts error:', e); }
}
setInterval(pollMapAlerts, 4000);
...
function showTab(name, el){
  ...
  if(name==='apitester') initApiTester();
  if(name==='map'){
    ...
    setTimeout(()=>{
      while(_pendingMapAlerts.length) showMapAlert(_pendingMapAlerts.shift());
    }, 400);
  }
}
...
async function testNotifications(){
  try{
    const r = await fetch(api('/notify-test'), {method:'POST', headers:{'Content-Type':'application/json'}, body:'{}'});
    const d = await r.json();
    if(d.error){ toast('Test failed: '+d.error, 'danger'); return; }
    if(!d.supervisor_token){
      toast('⚠️ No SUPERVISOR_TOKEN — check add-on config.yaml has hassio_api:true', 'warn');
    } else if(d.services_found && d.services_found.length){
      toast('✅ Sent to: '+d.services_found.join(', '), 'ok');
    } else {
      toast('⚠️ No notify services found in HA', 'warn');
    }
  }catch(e){ toast('Test error: '+e, 'danger'); }
}
...
async function clearMapLayout(){
  if(!confirm('Clear all device positions from the map? This cannot be undone.\n\nYour devices list and image are kept — only the layout is cleared.')) return;
  try{
    await fetch(api('/clear-map-layout'),{method:'POST',headers:{'Content-Type':'application/json'},body:'{}'});
    mapPositions={};
    mapConnections=[];
    connectSrc=null;
    activeTTNode=null;
    ...
    toast('Map layout cleared — drag devices from the sidebar to rebuild','ok');
  }catch{ toast('Clear failed','danger'); }
}
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if p=="/" or p=="":
            ...
            self.send_json(load_api_tester())
        elif p=="/map-alerts":
            with _map_alerts_lock:
                self.send_json(list(_map_alerts))
            ...
            # Fallback — serve the app for any unknown path (handles ingress path variations)
            html = HTML.replace("__INGRESS_BASE__", base)
            body = html.encode()
            self.send_response(200);self.send_header("Content-Type","text/html")
            self.send_header("Content-Length",len(body));self.end_headers();self.wfile.write(body)
    ...
    def do_POST(self):
        ...
        if p=="/run":
            ...
            try:
                services = _get_ha_notify_services()
                push_map_alert("online", "🔔 Test Alert", "Banner system is working!")
                threading.Thread(
                    target=send_ha_notification,
                    args=("🔔 NetMonitor Test", "Push notifications are working!", "netmon_test"),
                    daemon=True
                ).start()
                self.send_json({
                    "status": "ok",
            ...
            try:
                cfg = load_config()
                to = cfg.get("sms_to","")
                threading.Thread(target=send_sms, args=("🔔 NetMonitor test SMS — working!",), daemon=True).start()
                self.send_json({"status":"ok", "to": to})
            except Exception as e: self.send_json({"error":str(e)},400)
        elif p=="/notify-simulate":
            try:
                payload = json.loads(body)
                kind = payload.get("type","offline")
                name = payload.get("name","Test Device")
                ip   = payload.get("ip","10.0.0.1")
                if kind == "offline":
                    push_map_alert("offline", f"⚠️ Device Offline", f"{name} · {ip}")
                    msg = f"⚠️ Device Offline: {name} ({ip}) is not responding."
                    threading.Thread(target=send_ha_notification,
                        args=(f"⚠️ Device Offline", f"{name} ({ip}) is not responding.", "netmon_sim"),
                        daemon=True).start()
                    threading.Thread(target=send_sms, args=(msg,), daemon=True).start()
                else:
                    push_map_alert("online", f"✅ Device Back Online", f"{name} · {ip}")
                    msg = f"✅ Device Back Online: {name} ({ip}) is responding again."
                    threading.Thread(target=send_ha_notification,
                        args=(f"✅ Device Back Online", f"{name} ({ip}) is responding again.", "netmon_sim"),
                        daemon=True).start()
                    threading.Thread(target=send_sms, args=(msg,), daemon=True).start()
                self.send_json({"status":"ok"})
            except Exception as e: self.send_json({"error":str(e)},400)
        elif p=="/purge-results":
            try:
                _purge_stale_results(load_config())
                self.send_json({"status":"ok"})
            except Exception as e: self.send_json({"error":str(e)},400)
        elif p=="/config":
            ...
            try:
                ...
                # Save to history
                data = load_api_tester()
                data.setdefault("history",[]).insert(0, result)
                data["history"] = data["history"][:100]  # keep last 100
                save_api_tester(data)
                self.send_json(result)
            except Exception as e: self.send_json({"error":str(e)},400)
        elif p=="/api-tester/saved":
            try:
                save_api_tester(json.loads(body))
                self.send_json({"status":"ok"})
            except Exception as e: self.send_json({"error":str(e)},400)
        ...
        elif p=="/clear-map-layout":
            try:
                save_map_positions({"positions": {}, "connections": []})
                self.send_json({"status":"ok"})
            except Exception as e: self.send_json({"error":str(e)},400)
        elif p=="/map-image":
            try:
...
if __name__=="__main__":
    print("="*55)
    print("  NETMONITOR v5.9.10")
    print("="*55)
    # Purge any stale results on startup
    _purge_stale_results(load_config())
