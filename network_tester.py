        ...
        if not is_online:
            ...
            if count == threshold and prev is not False:
                ...
                threading.Thread(
                    ...
                    args=(
                        f"⚠️ {type_label} Offline",
                        f"{name} ({ip}) is not responding.",
                        f"netmon_{key}"
                    ),
                    daemon=True
                ).start()
        ...
        if now - _last_outage_alert_time > 600:  # 10 minute cooldown
            ...
            threading.Thread(
                ...
                args=(
                    ...
                    f"{recent_offline} devices are currently offline. Possible network outage.",
                    "netmon_outage"
                ),
                daemon=True
            ).start()
...
def notify_bandwidth_fail(url, error):
    """Send notification when bandwidth test fails."""
    push_map_alert("bw", "📉 Bandwidth Test Failed", error[:60])
    threading.Thread(
        target=send_ha_notification,
        args=(
            "📉 Bandwidth Test Failed",
            f"Download test could not complete. Error: {error[:80]}",
            "netmon_bandwidth"
        ),
        daemon=True
    ).start()
