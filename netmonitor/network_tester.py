...
def _check_mass_outage():
    """If 5+ devices went offline in the last 2 minutes, send a network outage alert."""
    recent_offline = sum(
        1 for k,v in _online_states.items()
        if v is False
    )
    if recent_offline >= 5:
        # Only fire once — check if we already sent this alert
        if _fail_counts.get("__outage_alerted__", 0) != recent_offline:
            _fail_counts["__outage_alerted__"] = recent_offline
...
#map-alert-stack{position:absolute;top:.5rem;left:50%;transform:translateX(-50%);
  display:flex;flex-direction:column;gap:.4rem;z-index:500;pointer-events:none;
  width:min(480px, calc(100% - 1rem));align-items:center;}
.map-alert{width:100%;background:rgba(5,10,18,.96);border-radius:8px;
  box-shadow:0 4px 24px rgba(0,0,0,.7);padding:.55rem .8rem;
  display:flex;align-items:center;gap:.65rem;
  opacity:0;transform:translateY(-12px);transition:all .3s ease;pointer-events:auto;
  border:1px solid rgba(255,255,255,.08);}
.map-alert.show{opacity:1;transform:translateY(0);}
.map-alert.ma-offline{border-left:3px solid var(--danger);}
.map-alert.ma-online {border-left:3px solid var(--accent2);}
.map-alert.ma-outage {border-left:3px solid #ff6b00;background:rgba(20,8,0,.97);}
.map-alert.ma-bw     {border-left:3px solid var(--warn);}
.map-alert-icon{font-size:1.1rem;flex-shrink:0;}
.map-alert-body{flex:1;min-width:0;}
.map-alert-title{font-family:'Exo 2';font-size:.72rem;font-weight:700;letter-spacing:.05em;margin-bottom:.1rem;}
.map-alert-msg  {font-family:'Share Tech Mono';font-size:.6rem;color:var(--dim);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;}
.map-alert-close{background:none;border:none;color:var(--dim);cursor:pointer;font-size:.85rem;
  padding:0 .2rem;flex-shrink:0;line-height:1;pointer-events:auto;}
.map-alert-close:hover{color:var(--text);}
.map-alert-age{font-family:'Share Tech Mono';font-size:.55rem;color:var(--dim);flex-shrink:0;}
...
function showMapAlert(alert){
  const stack = document.getElementById('map-alert-stack');
  if(!stack) return;

  const el = document.createElement('div');
  el.className = `map-alert ${ALERT_CLASS[alert.kind]||''}`;
  el.dataset.alertId = alert.id;

  const ts = new Date(alert.ts);
  const age = ts.toLocaleTimeString([], {hour:'2-digit',minute:'2-digit',second:'2-digit'});

  el.innerHTML = `
    <span class="map-alert-icon">${ALERT_ICONS[alert.kind]||'ℹ️'}</span>
    <div class="map-alert-body">
      <div class="map-alert-title" style="color:${ALERT_COLOR[alert.kind]||'var(--accent)'}">${alert.title}</div>
      <div class="map-alert-msg">${alert.msg}</div>
    </div>
    <span class="map-alert-age">${age}</span>
    <button class="map-alert-close" onclick="dismissMapAlert(this)" title="Dismiss">&#10005;</button>`;

  stack.appendChild(el);
  requestAnimationFrame(()=>{ el.classList.add('show'); });

  // Auto-dismiss: online/bw after 8s, offline after 20s, outage stays until dismissed
  const ttl = alert.kind==='outage' ? 0 : alert.kind==='offline' ? 20000 : 8000;
  if(ttl) setTimeout(()=>dismissMapAlert(el.querySelector('.map-alert-close')), ttl);
}

function dismissMapAlert(btn){
  const el = btn.closest('.map-alert');
  if(!el) return;
...
async function pollMapAlerts(){
  // Always poll — queue alerts even when map isn't active
  try{
    ...
    for(const a of alerts){
      if(!_seenAlertIds.has(a.id)){
        ...
        // Show immediately if map is active, otherwise queue for when map opens
        const mapActive = document.getElementById('page-map').classList.contains('active');
        if(mapActive){
          showMapAlert(a);
        } else {
          _pendingMapAlerts.push(a);
        }
      }
    }
