...
def send_ha_notification(title, message, tag=None):
    """Send push notification via HA Supervisor API."""
    try:
      ...
      <div class="mgr-card">
        ...
        <div class="add-form">
          ...
          <div style="display:flex;gap:.5rem;">
            ...
            <button class="btn btn-blue" onclick="testSms()">&#128241; Send Test SMS</button>
          </div>
        </div>
...
async function testSms(){
  try{
    ...
    if(d.error) toast('SMS test failed: '+d.error,'danger');
    else toast('✅ Test SMS sent!','ok');
  }catch(e){ toast('SMS test error: '+e,'danger'); }
}
...
async function testNotifications(){
  try{
    ...
    if(!d.supervisor_token){
      toast('⚠️ No SUPERVISOR_TOKEN — push notifications need add-on rebuild with hassio_api:true in config.yaml', 'warn');
    } else {
      toast('✅ Test sent — check your phone!', 'ok');
    }
    ...
    def do_POST(self):
        ...
        if p=="/run":
            ...
            try:
                ...
                self.send_json({"status":"ok","supervisor_token": bool(os.environ.get("SUPERVISOR_TOKEN",""))})
